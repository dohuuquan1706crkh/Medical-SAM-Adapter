...
def get_dataloader(args):
    ...
    if args.dataset == 'isic':
        ...
        '''drive data'''
        drive_train_dataset = DRIVE(args, args.data_path, transform = transform_train, transform_msk= transform_train_seg, mode = 'training')
        drive_test_dataset = DRIVE(args, args.data_path, transform = transform_test, transform_msk= transform_test_seg, mode = 'test')

        nice_train_loader = DataLoader(drive_train_dataset, batch_size=args.b, shuffle=True, pin_memory=True)
        nice_test_loader = DataLoader(drive_test_dataset, batch_size=args.b, shuffle=False, pin_memory=True)
        '''end'''

    elif args.dataset == 'drive_to_chasedb':
        '''drive data'''
        drive_train_dataset = DRIVE(args, args.data_path, transform = transform_train, transform_msk= transform_train_seg, mode = 'training')
        drive_test_dataset = DRIVE(args, args.data_path, transform = transform_test, transform_msk= transform_test_seg, mode = 'test')

        dataset = CHASEDB(args, data_path = "./data/CHASEDB1", transform = transform_train, transform_msk= transform_train_seg)
        ...
        test_sampler = SubsetRandomSampler(indices[:split])
        nice_test_loader_outDist = DataLoader(dataset, batch_size=args.b, sampler=test_sampler, pin_memory=True)
        ...
        '''chasedb data'''
        # dataset = LIDC(data_path = args.data_path)
        dataset = CHASEDB(args, data_path = args.data_path, transform = transform_train, transform_msk= transform_train_seg)
    ...
    elif args.dataset == 'chasedb_to_drive':
        '''chasedb data'''
        # dataset = LIDC(data_path = args.data_path)
        dataset = CHASEDB(args, data_path = args.data_path, transform = transform_train, transform_msk= transform_train_seg)
        ...
        # np.random.shuffle(indices)
        dump = indices[split:]
        np.random.shuffle(dump)
        train_sampler = SubsetRandomSampler(dump)
        # np.random.shuffle(train_sampler)
        test_sampler = SubsetRandomSampler(indices[:split])
        ...
        drive_test_dataset = DRIVE(args, "./data/DRIVE", transform = transform_test, transform_msk= transform_test_seg, mode = 'test')
        nice_test_loader_outDist = DataLoader(drive_test_dataset, batch_size=args.b, shuffle=False, pin_memory=True)
        '''end'''
